...
try {
    ...
    PreparedStatement statement = con.prepareStatement("SELECT * FROM applied where client_id in (select id from client where user_id in(select user_id from users where email=?)) and status='accepted' and work_status='completed' and pay_status='completed'");
...
%>
 ...
 <div class="col-sm-3 mx-5 my-3 mb-3 mb-sm-0">
            <div class="card" style="width: 18rem;">
                <div class="card-body">
                    ...
                    <p class="card-text">Work Status : <%=rs.getString("work_status") %></p>
                     <p class="card-text">Payment Status : <%=rs.getString("pay_status") %></p>
                     <p class="card-text">Payment Amount : <%=rs2.getString("payment_amount") %></p>
                    <span>
              <!--       <form action="paymentServlet" method="post" > -->
        				...
        				<%-- <a href="payment.jsp?id=<%=rs.getInt("id") %>&cid=<%=rs.getInt("client_id")%>&fid=<%=rs.getInt("free_id")%>"><button class="btn btn-primary">Make Payment</button></a> --%>
